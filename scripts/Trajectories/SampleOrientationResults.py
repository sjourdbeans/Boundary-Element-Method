...
folder=f"{main_folder}/mesh=320_shear={shear_rate}_N=4500_periods_140"

output_folder =f"{main_folder}/sampled"
Path(output_folder).mkdir(parents=True, exist_ok=True)
output_file = f"{output_folder}/mesh=320_shear={shear_rate}_N=4500_periods_140.h5"
...
for name, (shape, dtype) in dataset_info.items():
    if len(shape) >= 1 and shape[0] == frames:
        ...
        time_resolved[name] = np.zeros((N_conditions,) + shape, dtype=dtype)
    else:
        static_datasets[name] = np.zeros((N_conditions,) + shape, dtype=dtype)
...
with h5py.File(output_file, "w") as f_out:
    ...
    f_out.attrs["N_conditions"]    = N_conditions
