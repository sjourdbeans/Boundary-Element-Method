...
meshfile = "/home/sjoerd-buitjes/University/Master-Thesis/BEM/Boundary-Element-Method/datafiles/mesh/Chlamy/chlamy_N=320.mat"

mesh = bem.Mesh(meshfile)

sys=bem.MobilityProblem(mesh)



outdir = Path(f"/home/sjoerd-buitjes/University/Master-Thesis/Master-Thesis-Project/Data/BEM/python-BEM/trajectories/rigid-particles/ratio=1.25/shear={round(shear_rate,1)}_N={n_sims}_periods_{n_periods}")
...
with h5py.File(rank_file, "w") as h5:
    ...
    for sim_idx in local_indices:
        ...
        particle = copy.deepcopy(sys)

        sol = sys.RBM_over_time(
            dt,
            t_end,
