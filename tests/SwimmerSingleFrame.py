...
R[frame]=(rotmat @ (R[frame]-R[frame][0]).T ).T +np.array([12,2,0])
R2[frame]=(rotmat @ (R2[frame]-R2[frame][0]).T  ).T+np.array([12,-2,0])
...
# print(f"Total force: {np.sum(f1,axis=0)+ np.sum(f2,axis=0)}")
