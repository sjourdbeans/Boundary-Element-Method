...
# fileswimmer = "/home/sjoerd-buitjes/University/Master-Thesis/Master-Thesis-Project/Data/BEM/python-BEM/swimmer-objects/Chlamy/free/chlamy_free_3d_waveform.pkl"
fileswimmer=f"/home/sjoerd-buitjes/University/Master-Thesis/Master-Thesis-Project/Data/BEM/python-BEM/swimmer-objects/Euglena/Rossi/Free/Euglena_N=320_experimental.pkl"
...
folder = f"/home/sjoerd-buitjes/University/Master-Thesis/Master-Thesis-Project/Data/BEM/python-BEM/trajectories/euglena/shear={shear_rate}_N=2_periods_100"
...
discard_beats = 0
new_frames = 32* frames_per_beat#int(round(0.24*frames))
...
theta = np.arctan2(directors[:, :, 1], directors[:, :, 0])/np.pi  # azimuth
phi   = np.arcsin(np.clip(directors[:, :, 2], -1, 1))/np.pi        # elevation
...
for i, color in enumerate(colors):
    ax1.scatter(theta[i, 1:], phi[i, 1:],   # trajectory
                s=3, color=color, alpha=0.5)
    ax1.scatter(theta[i, 0],  phi[i, 0],    # initial condition
                s=80, color=color, marker='*', zorder=5)

ax1.set_xlabel('Azimuthal Angle $\\phi/\\pi$')
ax1.set_ylabel('Polar Angle $\\theta/\\pi$')
ax1.set_title(f'Angular Orientation with $\\dot{{\\gamma}}={shear_rate}$ s$^{{-1}}$')
ax1.set_xlim(-1, 1)
...
for i, color in enumerate(colors):
    ...
    ax2.scatter(x[0],  y[0],  z[0],  s=60, color=color, marker='*', zorder=5)

ax2.set_title('Orientation on the Unit Sphere')
ax2.set_xlabel('$x$'); ax2.set_ylabel('$y$'); ax2.set_zlabel('$z$')
ax2.view_init(elev=16, azim=-47)
# plt.tight_layout()
# plt.savefig(f"/home/sjoerd-buitjes/University/Master-Thesis/Master-Thesis-Project/plots/Orientations/chlamy/3D/Angular_orientation_and_sphere={shear_rate}.pdf")
# plt.savefig(f"/home/sjoerd-buitjes/University/Master-Thesis/Master-Thesis-Project/plot_images/Orientations/chlamy/3D/Angular_orientation_and_sphere={shear_rate}.png",dpi=600)
plt.show()