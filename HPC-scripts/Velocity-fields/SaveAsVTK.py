...
outdir = Path(f"/scratch/sbuitjes/simulation_results/velocity-fields/chlamy/chlamy-3d/Flowfield_Nx-Ny-Nz={Nx}-{Ny}-{Nz}_vtk_export")
...
with open(fileswimmer, "rb") as f:
    swimmer = pickle.load(f)

solution = swimmer.RBM_over_time(
    dt,
    swimmer.N_frames * dt,
    find_flow,
...
print(f"N_frames = {swimmer.N_frames}")
...
interaction = bem.FlowStokes(swimmer.mesh, points)


# =========================
# Save body mesh once
# =========================
panels = swimmer.mesh.panels[1:, :, :]
...
# for frame in range(swimmer.N_frames):
#     u_field = swimmer.calc_vector_field(
#         interaction,
#         frame,
#         find_flow,
#         include_rbm=False,
...
# =========================
for frame in range(swimmer.N_frames):
    print(f"Exporting frame {frame+1}/{swimmer.N_frames}")
    ...
    r1 = as_xyz_points(swimmer.flagellum_1[frame].r)
    r2 = as_xyz_points(swimmer.flagellum_2[frame].r)
