...
# Use the three lines below for symmetric swimmer
# R2 = R.copy()
# R2[:,:,0] *= -1
# R2[:,:,2] *= -1
...
for scale_out_of_plane in scales:
    ...
    for time_frame in range(Nt):
        ...
        curve_1 = bem.SlenderAngles(theta_1_map[time_frame,:], scale_out_of_plane*phi_1_map[time_frame,:], flagellum_length=lf,flagellum_radius=0.1)
        curve_2 = bem.SlenderAngles(theta_2_map[time_frame,:], scale_out_of_plane*phi_2_map[time_frame,:], flagellum_length=lf,flagellum_radius=0.1)
